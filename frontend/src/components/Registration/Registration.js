...
class Registration extends React.Component {
  ...
  nextStep(e) {
    e.preventDefault();
    // would be great if this 2 was a variable, so it is easier to stay consistent across components if you depend in many places on it
  ...
  handleSubmit(e) {
    ...
    // move it to services
    axios
      .post("http://192.168.8.192:8080/register", { data: this.state })
      .then(res => {
    ...
    if (cookie.get("login")) {
      ...
      return (
        <div className="registration">
          {/* you can just do {this.state.visible && <Alert .... /> }*/}
          {this.state.visible ? (
            <Alert
